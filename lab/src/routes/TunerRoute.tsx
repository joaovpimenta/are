...
export function TunerRoute({ entry, theme, reducedMotion, resetVersion, session }: LabRouteProps) {
  ...
  useEffect(() => {
    session.send({ type: solved ? 'MECHANISM_SOLVED' : 'MECHANISM_ACTIVE', mechanismId: 'tuner' });
  }, [session, solved]);
  useEffect(() => {
  ...
  const step = (direction: number) => setValue((current) => stepDialValue(current, direction, { min: 0, max: 99 }));
  return (
    <MechanismPage
      entry={entry}
      status={solved ? 'solved' : 'active'}
      visual={<HardwareCanvas ariaLabel="Rádio de ondas curtas 3D" cameraZ={7.8}><Tuner3D value={value} target={73} theme={theme} reducedMotion={reducedMotion} onChange={setValue} /></HardwareCanvas>}
      controls={<><button {...stylex.props(labStyles.controlButton)} type="button" aria-label="Diminuir frequência" onClick={() => step(-1)}>−</button><input {...stylex.props(labStyles.slider)} type="range" min="0" max="99" value={value} aria-label="Frequência" onChange={(event) => setValue(Number(event.currentTarget.value))} /><output {...stylex.props(labStyles.controlReadout)}>{String(value).padStart(2, '0')}</output><button {...stylex.props(labStyles.controlButton)} type="button" aria-label="Aumentar frequência" onClick={() => step(1)}>+</button></>}
